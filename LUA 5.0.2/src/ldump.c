...
static void DumpBlock(const void* b, lua_Size size, DumpState* D)
{
 lua_unlock(D->L);
...
static void DumpSize(lua_Size x, DumpState* D)
{
 DumpBlock(&x,sizeof(x),D);
}
...
static void DumpString(TString* s, DumpState* D)
{
 ...
 {
  lua_Size size=s->tsv.len+1;		/* include trailing '\0' */
  DumpSize(size,D);
...
static void DumpHeader(DumpState* D)
{
 ...
 //DumpByte(sizeof(size_t),D); // mzhk
 DumpByte(sizeof(lua_Size),D);
