...
LUALIB_API const char *luaL_checklstring (lua_State *L, int narg, lua_Size *len) {
...
#define bufffree(B)	((lua_Size)(LUAL_BUFFERSIZE - bufflen(B)))

#define LIMIT	(LUA_MINSTACK/2)


static int emptybuffer (luaL_Buffer *B) {
  lua_Size l = bufflen(B);
...
static void adjuststack (luaL_Buffer *B) {
  if (B->lvl > 1) {
    ...
    lua_Size toplen = lua_strlen(L, -1);
    do {
      lua_Size l = lua_strlen(L, -(toget+1));
...
LUALIB_API void luaL_addlstring (luaL_Buffer *B, const char *s, lua_Size l) {
  while (l--)
    luaL_putchar(B, *s++);
}
...
static const char *getF (lua_State *L, void *ud, lua_Size *size) {
  LoadF *lf = (LoadF *)ud;
  (void)L;
...
static const char *getS (lua_State *L, void *ud, lua_Size *size) {
  LoadS *ls = (LoadS *)ud;
  (void)L;
...
LUALIB_API int luaL_loadbuffer (lua_State *L, const char *buff, lua_Size size,
                                const char *name) {
  LoadS ls;
  ls.s = buff;
  ls.size = size;
...
LUALIB_API int lua_dobuffer (lua_State *L, const char *buff, lua_Size size,
                          const char *name) {
