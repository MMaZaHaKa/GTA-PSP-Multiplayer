...
static int read_chars (lua_State *L, FILE *f, lua_Size n) {
  lua_Size rlen;  /* how much to read */
  lua_Size nr;  /* number of chars actually read */
  luaL_Buffer b;
  luaL_buffinit(L, &b);
  ...
  if (nargs == 0) {  /* no arguments? */
    ...
    for (n = first; nargs-- && success; n++) {
      if (lua_type(L, n) == LUA_TNUMBER) {
        lua_Size l = (lua_Size)lua_tonumber(L, n);
        ...
        switch (p[1]) {
          ...
          case 'l':  /* line */
            ...
            break;
          case 'a':  /* file */
            read_chars(L, f, ~((lua_Size)0));  /* read MAX_SIZE_T chars */
