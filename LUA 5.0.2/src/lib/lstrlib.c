...
static sint32 posrelat (sint32 pos, lua_Size len) {
...
static int str_byte (lua_State *L) {
  ...
  if (pos <= 0 || (lua_Size)(pos) > l)  /* index out of range? */
    return 0;  /* no answer */
...
static int writer (lua_State *L, const void* b, lua_Size size, void* B) {
  (void)L;
...
static const char *lmemfind (const char *s1, lua_Size l1,
                               const char *s2, lua_Size l2) {
...
static int str_find (lua_State *L) {
  ...
  if (init < 0) init = 0;
  else if ((lua_Size)(init) > l1) init = (sint32)l1;
...
static int gfind_aux (lua_State *L) {
  ...
  lua_Size ls = lua_strlen(L, lua_upvalueindex(1));
  const char *p = lua_tostring(L, lua_upvalueindex(2));
  const char *src;
  ms.L = L;
  ms.src_init = s;
  ms.src_end = s+ls;
  for (src = s + (lua_Size)lua_tonumber(L, lua_upvalueindex(3));
       src <= ms.src_end;
       src++) {
    const char *e;
    ms.level = 0;
