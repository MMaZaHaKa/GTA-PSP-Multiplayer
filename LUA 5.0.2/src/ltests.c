...
#define setsize(newblock, size)	(*cast(lua_Size *, newblock) = size)
#define checkblocksize(b, size) (size == (*cast(lua_Size *, blockhead(b))))
...
static void *checkblock (void *block, lua_Size size) {
...
static void freeblock (void *block, lua_Size size) {
  if (block) {
...
void *debug_realloc (void *block, lua_Size oldsize, lua_Size size) {
  ...
  if (size == 0) {
    ...
    void *newblock;
    int i;
    lua_Size realsize = HEADER+size+MARKSIZE;
    lua_Size commonsize = (oldsize < size) ? oldsize : size;
...
static int newuserdata (lua_State *L) {
  lua_Size size = luaL_checkint(L, 1);
