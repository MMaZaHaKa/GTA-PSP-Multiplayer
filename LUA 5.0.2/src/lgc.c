...
lua_Size luaC_separateudata (lua_State *L) {
  lua_Size deadmem = 0;
...
static void checkSizes (lua_State *L, lua_Size deadmem) {
  ...
  if (luaZ_sizebuffer(&G(L)->buff) > LUA_MINBUFFER*2) {  /* buffer too big? */
    lua_Size newsize = luaZ_sizebuffer(&G(L)->buff) / 2;
...
static lua_Size mark (lua_State *L) {
  lua_Size deadmem;
  GCState st;
  GCObject *wkv;
  st.g = G(L);
