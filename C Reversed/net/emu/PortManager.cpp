...
#ifdef WITH_UPNP
	if (!g_Config.bEnableUPnP) {
		INFO_LOG(Log::sceNet, "__UPnPInit(timeout_ms:%d) skipped. bEnableUPnP false\n", timeout_ms);
		return;
	}
#endif
...
void __UPnPShutdown() {
#ifdef WITH_UPNP
	if (!g_Config.bEnableUPnP) {
		INFO_LOG(Log::sceNet, "__UPnPShutdown skipped. bEnableUPnP false\n");
		return;
	}
#endif
	...
	//g_PortManager.Shutdown(); // from NativeShutdown (why)
}

void UPnP_Add(const char* protocol, unsigned short port, unsigned short intport) {
#ifdef WITH_UPNP
	if (!g_Config.bEnableUPnP) {
		INFO_LOG(Log::sceNet, "UPnP_Add(port:%d, intport:%d) skipped. bEnableUPnP false\n", port, intport);
		return;
	}
#endif
...
#ifdef WITH_UPNP
	if (!g_Config.bEnableUPnP) {
		INFO_LOG(Log::sceNet, "UPnP_Remove(protocol:%s, port:%d) skipped. bEnableUPnP false\n", protocol, port);
		return;
	}
#endif
