...
SceNetAdhocctlPeerInfo * localPeerInfo = NULL;
SceNetAdhocctlPeerInfo * friends       = NULL;
SceNetAdhocctlScanInfo * networks      = NULL;
SceNetAdhocctlScanInfo * newnetworks   = NULL;
uint64_t adhocctlStartTime             = 0;

//bool isAdhocctlNeedLogin              = false;
//bool isAdhocctlBusy                   = false;
std::atomic<bool> isAdhocctlNeedLogin  = false; // for waiter in other thread
std::atomic<bool> isAdhocctlBusy       = false;
...
uint8_t* gameModeBuffer                = nullptr;
...
std::map<SceNetEtherAddr, uint16_t> gameModePeerPorts;
...
bool isLocalServer = false; // localhost or 127.
...
std::vector<ChatMessageCB> chatCallbacks;
void AddChatMessageCallback(ChatMessageCB cb) {
	//std::lock_guard<std::mutex> lg(mutex);
	chatCallbacks.push_back(std::move(cb));
}
void OnChatMessage(const std::string& nickname, const std::string& message) {
	std::vector<ChatMessageCB> copy;
	{
		//std::lock_guard<std::mutex> lg(mutex);
		copy = chatCallbacks;
	}

	// TODO? strstr SERVER_SHUTDOWN_MESSAGE OnServerShutdown, (bug can manual send SERVER_SHUTDOWN_MESSAGE)

	for (auto& cb : copy) {
		if (cb) cb(nickname, message);
	}
}
...
void addFriend(SceNetAdhocctlConnectPacketS2C * packet) {
	...
	bool bIsCustom = (packet->name.data[ADHOCCTL_NICKNAME_LEN - strlen(ADHOCCTL_CUSTOM_IDENT) - 2] == '\0' && // prevent long nickname bug
		memcmp((char*)&packet->name.data + ADHOCCTL_NICKNAME_LEN - strlen(ADHOCCTL_CUSTOM_IDENT) - 1, ADHOCCTL_CUSTOM_IDENT, strlen(ADHOCCTL_CUSTOM_IDENT)) == 0);
	if (peer && bIsCustom)
		peer->flags |= ADHOCCTL_CUSTOM_FLAG;
#endif
}
...
// extention
SceNetAdhocctlPeerInfo* findSelf() { return localPeerInfo; }
SceNetAdhocctlPeerInfo* getPeerInfo(SceNetEtherAddr* MAC, bool* bIsLocal) {
	if (bIsLocal) *bIsLocal = false;
	SceNetAdhocctlPeerInfo* local = findSelf();
	if (local && isMacMatch(&local->mac_addr, MAC)) {
		if (bIsLocal) *bIsLocal = true;
		return local;
	}
	return findFriend(MAC);
}
...
bool InitLocalPeerInfo()
{
	FreeLocalPeerInfo();

	localPeerInfo = (SceNetAdhocctlPeerInfo*)malloc(sizeof(SceNetAdhocctlPeerInfo));
	if (localPeerInfo == NULL) {
		ERROR_LOG(Log::sceNet, "InitLocalPeerInfo: malloc failed");
		return false;
	}

	memset(localPeerInfo, 0, sizeof(SceNetAdhocctlPeerInfo));

	// Self is not part of friends list, so store local values here
	localPeerInfo->nickname = parameter.nickname;
	getLocalMac(&localPeerInfo->mac_addr);

	sockaddr_in localAddr{};
	if (getLocalIp(&localAddr) == 0) {
		localPeerInfo->ip_addr = localAddr.sin_addr.s_addr;
	}

	localPeerInfo->padding = 0;
#ifdef ADHOCCTL_USE_CUSTOM_IDENT
	localPeerInfo->flags = 0 | ADHOCCTL_CUSTOM_FLAG;
#else
	localPeerInfo->flags = 0;
#endif
	localPeerInfo->last_recv = CoreTiming::GetGlobalTimeUsScaled();

	// For self keep the same behavior as resolveMAC(localMac)
	localPeerInfo->port_offset = portOffset;

	localPeerInfo->next = NULL;
	return true;
}
...
*/
int32_t countChildren(SceNetAdhocMatchingContext * context, const bool excludeTimedout)
{
	// Children Counter
	int32_t count = 0;
...
void handleTimeout(SceNetAdhocMatchingContext * context)
{
	...
	// Iterate Peer List
	SceNetAdhocMatchingMemberInternal * peer = context->peerlist; 
	while (peer != NULL && contexts != NULL)
	{
		// Get Next Pointer (to avoid crash on memory freeing)
		SceNetAdhocMatchingMemberInternal * next = peer->next;

		uint64_t now = CoreTiming::GetGlobalTimeUsScaled(); //time_now_d()*1000000.0
		...
		if (peer->state != 0 && static_cast<int64_t>(now - peer->lastping) > static_cast<int64_t>(context->timeout)) 
		{
	...
	if (friendFinderRunning) {
		...
		if (!chatString.empty()) {
			...
			if (IsSocketReady((int)metasocket, false, true) > 0) {
				...
				OnChatMessage(name, chat.message);
				return true;
			}
		}
	}
		...
		std::lock_guard<std::mutex> guard(chatLogLock);
		chatLog.push_back(std::string("You're in Offline Mode, go to lobby or online hall"));
		chatMessageGeneration++;

		OnChatMessage(g_Config.sNickName, "You're in Offline Mode, go to lobby or online hall");
	}
...
int friendFinder() {
	...
	if (g_Config.bEnableWlan && !net::DNSResolve(g_Config.sProAdhocServer, "", &resolved, err)) {
		assert(false && "bad host addr in config");
	...
	g_adhocServerIP.in.sin_port = htons(ADHOC_SERVER_PORT);
	...
	while (friendFinderRunning) {
		...
		if (g_adhocServerConnected) {
			...
			if (static_cast<int64_t>(now - lastping) >= PSP_ADHOCCTL_PING_TIMEOUT) { // We may need to use lower interval to prevent getting timeout at Pro Adhoc Server through internet
				// Prepare Packet
			...
			//if (isAdhocctlBusy && adhocctlState == ADHOCCTL_STATE_DISCONNECTED && adhocctlCurrentMode == ADHOCCTL_MODE_GAMEMODE && netAdhocGameModeEntered && static_cast<int64_t>(now - adhocctlStartTime) > netAdhocEnterGameModeTimeout) {
			//	netAdhocGameModeEntered = false;
			...
			if (rxpos > 0) {
				...
				// Chat Packet
					...
					if (rxpos >= (int)sizeof(SceNetAdhocctlChatPDPPTPPacketS2C) &&
						((SceNetAdhocctlChatPDPPTPPacketS2C*)rx)->base.subopcode == OPCODE_MZHK) {
						// Cast Packet
						SceNetAdhocctlChatPDPPTPPacketS2C* packet = (SceNetAdhocctlChatPDPPTPPacketS2C*)rx;
						INFO_LOG(Log::sceNet, "FriendFinder: Incoming OPCODE_CHAT [OPCODE_MZHK]");

						onRecvP2PChatBuffer(rx);

						// Move RX Buffer
						memmove(rx, rx + sizeof(SceNetAdhocctlChatPDPPTPPacketS2C), sizeof(rx) - sizeof(SceNetAdhocctlChatPDPPTPPacketS2C));

						// Fix RX Buffer Length
						rxpos -= sizeof(SceNetAdhocctlChatPDPPTPPacketS2C);
					}
					else
#endif
					// Enough Data available
					if (rxpos >= (int)sizeof(SceNetAdhocctlChatPacketS2C)) {
						...
						std::string chatmessage = (char*)packet->base.message;
						incoming.append(name.substr(0, 8));
						incoming.append(": ");
						incoming.append(chatmessage);
						...
						OnChatMessage(name, chatmessage);
...
#ifdef ADHOC_PTP_PDP_WRAPPERS
				// CSC Packet
				else if (rx[0] == OPCODE_MZHK) {
					// Enough Data for Header
					if (rxpos >= (int)sizeof(SceNetAdhocctlPDPPTPPacketS2C)) {
						// Cast Packet
						SceNetAdhocctlPDPPTPPacketS2C* packet = (SceNetAdhocctlPDPPTPPacketS2C*)rx;
						// Data Size
						uint16_t datasize = packet->base.size;
						// Total Packet Size
						size_t packetsize = sizeof(SceNetAdhocctlPDPPTPPacketS2C) + ((datasize > 0 && datasize < 65535) ? datasize : 0);
						// Enough Data for Full Packet
						if (rxpos >= (int)packetsize) {
							INFO_LOG(Log::sceNet, "FriendFinder: Incoming OPCODE_MZHK");
							onRecvP2PBuffer(rx, (int)packetsize);
							// Move RX Buffer
							memmove(rx, rx + packetsize, sizeof(rx) - packetsize);
							// Fix RX Buffer Length
							rxpos -= packetsize;
						}
					}
				...
				// Connect Packet
					...
					if (rxpos >= (int)sizeof(SceNetAdhocctlConnectPacketS2C)) {
						...
						OnChatMessage(ADHOC_SERVER_NAME, incoming);
...
#ifndef MAZAHAKA_PC // moved into read cfg
	if (GAME_ID > 1) {
		memset(&mac, GAME_ID, sizeof(mac));
...
int joinIPv4MulticastGroup(SOCKET s, const char* mcastAddr, const char* ifIp) {
	if (!mcastAddr || !ifIp) return SOCKET_ERROR;

	ip_mreq mreq;
	// multicast address
	if (inet_pton(AF_INET, mcastAddr, &mreq.imr_multiaddr) != 1) {
		return SOCKET_ERROR;
	}
	// interface address (local IP to receive on)
	if (inet_pton(AF_INET, ifIp, &mreq.imr_interface) != 1) {
		return SOCKET_ERROR;
	}
	...
	// ensure loopback so local sender receives its own packets (optional)
	unsigned char loop = 1;
	if (setsockopt(s, IPPROTO_IP, IP_MULTICAST_LOOP, (char*)&loop, sizeof(loop)) == SOCKET_ERROR) {
		// not fatal
	}

	INFO_LOG(Log::sceNet, "Joined multicast %s on interface %s", mcastAddr, ifIp);
	return 0;
}

int setIPv4MulticastOutgoingInterface(SOCKET s, const char* ifIp) {
	if (!ifIp) return SOCKET_ERROR;
	in_addr local_if;
	if (inet_pton(AF_INET, ifIp, &local_if) != 1) {
		return SOCKET_ERROR;
	}

	if (setsockopt(s, IPPROTO_IP, IP_MULTICAST_IF, (char*)&local_if, sizeof(local_if)) == SOCKET_ERROR) {
		return SOCKET_ERROR;
	}

	// make sure sender loopback on so local listeners see it
	unsigned char loop = 1;
	if (setsockopt(s, IPPROTO_IP, IP_MULTICAST_LOOP, (char*)&loop, sizeof(loop)) == SOCKET_ERROR) {
		// not fatal
	}

	INFO_LOG(Log::sceNet, "Set multicast outgoing interface to %s", ifIp);
	return 0;
}
...
int initNetwork(SceNetAdhocctlAdhocId *adhoc_id){
	...
	if (iResult == SOCKET_ERROR && errorcode != EISCONN) {
		...
		while (!done) {
			...
			if (static_cast<int64_t>(now - startTime) > adhocDefaultTimeout) {
		...
		if (!done) {
			ON_SOCKET_ERROR();
			assert(false && "metasocket error");
			ERROR_LOG(Log::sceNet, "Socket error (%i) when connecting to AdhocServer [%s/%s:%u]",
				errorcode, g_Config.sProAdhocServer.c_str(), ip2str(g_adhocServerIP.in.sin_addr).c_str(), ntohs(g_adhocServerIP.in.sin_port));
			return iResult;
		}
	}
	...
	packet.name.data[ADHOCCTL_NICKNAME_LEN - 1] = 0;
#ifdef ADHOCCTL_USE_CUSTOM_IDENT
	packet.name.data[ADHOCCTL_NICKNAME_LEN - strlen(ADHOCCTL_CUSTOM_IDENT) - 2] = '\0'; // trim long nickname
	strcpy((char*)&packet.name.data[ADHOCCTL_NICKNAME_LEN - strlen(ADHOCCTL_CUSTOM_IDENT) - 1], ADHOCCTL_CUSTOM_IDENT); // right align
#endif
...
// найти ip по сгенерированному mac юзверя
bool resolveMAC(SceNetEtherAddr* mac, uint32_t* ip, uint16_t* port_offset) {
